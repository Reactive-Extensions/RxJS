(function () {
  ...
  test('GroupBy_Outer_Complete', function () {
    ...
    results.messages.assertEqual(
      onNext(220, 'foo'),
      onNext(270, 'bar'),
      onNext(350, 'baz'), onNext(360, 'qux'), onCompleted(570));
  ...
  test('GroupBy_Inner_Error', function () {
      ...
      ex = 'ex1';
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(90, 'error'), onNext(110, 'error'), onNext(130, 'error'), onNext(220, '  foo'), onNext(240, ' FoO '), onNext(270, 'baR  '), onNext(310, 'foO '), onNext(350, ' Baz   '), onNext(360, '  qux '), onNext(390, '   bar'), onNext(420, ' BAR  '), onNext(470, 'FOO '), onNext(480, 'baz  '), onNext(510, ' bAZ '), onNext(530, '    fOo    '), onError(570, ex), onNext(580, 'error'), onCompleted(600), onError(650, new Error()));
      outer = null;
      outerSubscription = null;
      inners = {};
      innerSubscriptions = {};
  ...
  test('GroupBy_Inner_Escape_Complete', function () {
      ...
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(220, '  foo'), onNext(240, ' FoO '), onNext(310, 'foO '), onNext(470, 'FOO '), onNext(530, '    fOo    '), onCompleted(570));
  ...
  test('GroupBy_Inner_Escape_Error', function () {
      ...
      var ex = new Error();
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(220, '  foo'), onNext(240, ' FoO '), onNext(310, 'foO '), onNext(470, 'FOO '), onNext(530, '    fOo    '), onError(570, ex));
      results = scheduler.createObserver();
      scheduler.scheduleAbsolute(created, function () {
          outer = xs.groupBy(function (x) {
              return x.toLowerCase().trim();
  ...
  test('GroupBy_Inner_Escape_Dispose', function () {
      var inner, innerSubscription, outer, outerSubscription, results, scheduler, xs;
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(220, '  foo'), onNext(240, ' FoO '), onNext(310, 'foO '), onNext(470, 'FOO '), onNext(530, '    fOo    '), onError(570, new Error()));
      results = scheduler.createObserver();
      scheduler.scheduleAbsolute(created, function () {
          outer = xs.groupBy(function (x) {
              return x.toLowerCase().trim();

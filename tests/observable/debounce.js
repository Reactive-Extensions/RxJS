...
(function () {
  ...
  test('debounce TimeSpan AllPass ErrorEnd', function () {
      var error = new Error();
  ...
  test('debounce TimeSpan AllDrop', function () {
      var scheduler = new TestScheduler();
  ...
  test('debounce Duration debounceBehavior', function () {
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(150, -1), onNext(250, 0), onNext(280, 1), onNext(310, 2), onNext(350, 3), onNext(400, 4), onCompleted(550));
      var ys = [scheduler.createColdObservable(onNext(20, 42), onNext(25, 99)), scheduler.createColdObservable(onNext(40, 42), onNext(45, 99)), scheduler.createColdObservable(onNext(20, 42), onNext(25, 99)), scheduler.createColdObservable(onNext(60, 42), onNext(65, 99)), scheduler.createColdObservable(onNext(20, 42), onNext(25, 99))];
      var results = scheduler.startScheduler(function () {
  ...
  test('debounce Duration EarlyCompletion', function () {
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(150, -1), onNext(250, 0), onNext(280, 1), onNext(310, 2), onNext(350, 3), onNext(400, 4), onCompleted(410));
      var ys = [scheduler.createColdObservable(onNext(20, 42), onNext(25, 99)), scheduler.createColdObservable(onNext(40, 42), onNext(45, 99)), scheduler.createColdObservable(onNext(20, 42), onNext(25, 99)), scheduler.createColdObservable(onNext(60, 42), onNext(65, 99)), scheduler.createColdObservable(onNext(20, 42), onNext(25, 99))];
      var results = scheduler.startScheduler(function () {
          return xs.debounce(function (x) {
              return ys[x];
  ...
  test('debounce Duration InnerError', function () {
      var scheduler = new TestScheduler();
      ...
      results.messages.assertEqual(onNext(250 + 2 * 10, 2), onNext(350 + 3 * 10, 3), onError(450 + 4 * 10, error));
      xs.subscriptions.assertEqual(subscribe(200, 490));
  });

  test('debounce Duration OuterError', function () {
      var error = new Error();
      var scheduler = new TestScheduler();
      var xs = scheduler.createHotObservable(onNext(150, 1), onNext(250, 2), onNext(350, 3), onNext(450, 4), onError(460, error));
      var results = scheduler.startScheduler(function () {
          return xs.debounce(function (x) {
              return scheduler.createColdObservable(onNext(x * 10, 'Ignore'), onNext(x * 10 + 5, 'Aargh!'));
          });
      });
      results.messages.assertEqual(onNext(250 + 2 * 10, 2), onNext(350 + 3 * 10, 3), onError(460, error));
